...
C        tmptime = mpi_wtime()
        ...
        IF(firstc) THEN
         ...
         ! outf11 = '../../../../../step2_Nbody_simulation/'
         ! outf12 = 'gadget/Gadget-2.0.7/galaxy_general/intermediate/'
         ! outf2 = outf11//outf12//outcfile
           IF(outpcoef .and. me.eq.0)
     &          OPEN(uoutcoef,FILE=outcfile,STATUS='UNKNOWN')
C   &          OPEN(uoutcoef,FILE=outf2,STATUS='UNKNOWN')
C  gjy changed: change file for all snapshot
 ...
 100       FORMAT(1x,10(1pe22.13))
        ...
        REWIND(uincoef)
           if (me .eq. 0) then
            READ(uincoef,*) tt, mbh
            tmpsum01 = tt
C            READ(uincoef,*,iostat=ios) tt, mbh
C            if(ios.ne.0)exit
Cgjy changed
