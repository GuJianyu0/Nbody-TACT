...
C        CALL startout
        me = 0
C       _me_ means my rank in original mpi_ver,
C       single CPU ver, just set 0
C            --------
        CALL inparams
C            --------
        CALL inbods
...
C        DO 120 k=1,nbodies-1
        DO 120 k=1,nbodies

           IF (mstpflag.AND.im(k).EQ.1) GO TO 120
...
crss     (iii) ultraspt changed to ultrasp
     ...
     &                      temp4*ultrasp(n,l)
           ...
           if(im(k).eq.1) then
              DO l=lmin,lmax,lskip
                 DO m=0,l
                    ttemp5=temp5*plm(l,m) !rss
                    temp3=ttemp5*sinmphi(m)
                    temp4=ttemp5*cosmphi(m)
                    DO n=0,nmax
                       sinsum2(n,l,m)=sinsum2(n,l,m) +
     &                      temp3*ultrasp(n,l)
                       cossum2(n,l,m)=cossum2(n,l,m) +
     &                      temp4*ultrasp(n,l)
                    enddo
                 enddo
                 temp5=temp5*ratio
...
ccompaq     tmpsum replaced with tmpsum_snd
              ...
              sinsum(n,l,m) = sinsum1(n,l,m)
              cossum(n,l,m) = cossum1(n,l,m)
...
ccompaq     tmpsum replaced with tmpsum_snd
                ...
                sinsum(n,l,m) = sinsum(n,l,m)
     &                        + sinsum2(n,l,m)
                cossum(n,l,m) = cossum(n,l,m)
     &                        + cossum2(n,l,m)
	      endif
...
C      call MPI_Allreduce (tmpsum_snd,tmpsum_rcv, icount, 
C     &                    mpi_double_precision,
C     &                    mpi_sum, mpi_comm_world,ierr)
C        icount=0
C        DO 1182 l=lmin,lmax,lskip
C          DO 1162 m=0,l
C            DO 1142 n=0,nmax
C             icount=icount+2
...
C              sinsum(n,l,m) = tmpsum_rcv(icount-1)
C              cossum(n,l,m) = tmpsum_rcv(icount)

C 1142       CONTINUE
C 1162     CONTINUE
C 1182   CONTINUE

C        IF(MOD(nstep,noutlog).EQ.0.AND..NOT.mstpflag) THEN
         IF(outpcoef) CALL iocoef(sinsum,cossum)
C                                     ------
C        ENDIF
C        IF(inptcoef) CALL iocoef(sinsum,cossum)
        



C================== Calc pot and force ==================
        WRITE(*,*) 'Calc pot and force...'

C        DO 200 k=1,nbodies-1
        DO 200 k=1,nbodies
           IF (mstpflag.AND.im(k).EQ.1) GO TO 200
...
C
C
C     Subroutine to output potential data
C
C
C=======================================================================
...
C=======================================================================
        ...
        OPEN(uoutcoef,FILE='pot_scf.dat')

        WRITE(uoutcoef,100) tnow

C        DO 10 n=1,nbodies-1
        DO 10 n=1,nbodies
          WRITE(uoutcoef,101) n-1, pot(n)
 10     CONTINUE

 100    FORMAT(2x,1pe11.3)
 101    FORMAT(i6.6,2x,1pe11.3)
...
C
C
                     SUBROUTINE out_force
C
C
C***********************************************************************
C
C
C     Subroutine to output force data
C
C
C=======================================================================
...
C=======================================================================
        ...
        OPEN(uoutcoef,FILE='force_scf.dat')

        WRITE(uoutcoef,100) tnow

C        DO 10 n=1,nbodies-1
        DO 10 n=1,nbodies
          WRITE(uoutcoef,101) n-1, ax(n), ay(n), az(n)
 10     CONTINUE

 100    FORMAT(2x,1pe11.3)
 101    FORMAT(i6.6,2x,3(1pe15.7))
...
c            READ(ubodsin,*) nbodies,tnow, bhmass
            ...
            READ(ubodsin,*) tnow
            WRITE(*,*) 'nbodies: ', itmp, nbodies, tnow
C           gjy add
        ENDIF
        IF (nbodies .gt. nbodsmax) THEN
           IF (me .eq. 0) CLOSE(ubodsin)
           WRITE(*,*) ' Number of bodies exceeds the upper bound. '
           STOP
        ENDIF

C      call shareint(nbodies,1)
C      call share(tnow,1)

c       IF (MOD(nbodies, n_pes) .ne. 0) THEN
c          IF (me .eq. 0) CLOSE(ubodsin)
c          CALL terror(' Bodies cannot be distributed to PEs evenly. ')
c       ENDIF
...
c       nbodies = nbodies*n_pes
        ...
        IF (nbodies .gt. nbodsper) THEN
           IF (me .eq. 0) CLOSE(ubodsin)
           WRITE(*,*) ' Number of bodies per proc exceeds bound. '
           STOP
        ENDIF

        IF (me .eq. 0) THEN
            DO 10 i=1,nbodies
               READ(ubodsin,*,end=11) pid,mass(i),x(i),y(i),z(i),
     &                                vx(i),vy(i),vz(i)
 10    CONTINUE
        ENDIF
...
C           IF (me .eq. 0) THEN
C               DO 20 i=1,nbodies
C                  READ(ubodsin,*) temp01(i),temp02(i),temp03(i),
C     &                             temp04(i),temp05(i),temp06(i),
C     &                             temp07(i)
C20             CONTINUE
C           ENDIF

C               CALL mpiget(mass, temp01, nbodies,j,0,me)
C               CALL mpiget(x,    temp02, nbodies,j,0,me)
C               CALL mpiget(y,    temp03, nbodies,j,0,me)
C               CALL mpiget(z,    temp04, nbodies,j,0,me)
C               CALL mpiget(vx,   temp05, nbodies,j,0,me)
C               CALL mpiget(vy,   temp06, nbodies,j,0,me)
C               CALL mpiget(vz,   temp07, nbodies,j,0,me)
C           IF (me .eq. j) THEN
C           ENDIF
C30      CONTINUE

 11     IF (me .eq. 0) CLOSE(ubodsin)
        DO 40 i=1,nbodies
          im(i)=1
 40     CONTINUE
