...
! 确定大致的搜索范围
  ...
  call get_pot(x,pot)
  vmod = sqrt(abs(pot)) !gjy comment: original; or sqrt(abs(E-pot)), but only an init con
  print*,"y =",x(2), "v_circ=",vmod
...
! subroutine Integrator( x0, v0, outflag, dy )
  subroutine Integrator( x0, v0, outflag, dy, Fname_orbit ) !gjy change
   implicit none
!			/* parameter define */ !gjy comment: total time each
   integer outflag
...
! t_end = 10.0 ! Gadget unit: Gyr !gjy comment: original
...
close(15)
!gjy note: in this prog, here we ignore to control files 'Integrate.status', 
!\ 'NaN-record' and 'STOP', whose writing may make a mess in MPI


! 循环结束的三个控制条件： !gjy comment: total time each
! t_end 最大演化时间，Gyr
! n_apo 粒子经过远心点的次数
...
print*,"t_global = ", t_global !gjy add !gjy comment: total time each
! print*,"n_apo = ", n_apo !gjy add
