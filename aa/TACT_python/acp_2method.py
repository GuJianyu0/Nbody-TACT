...
def method_comparison(results_file,name,dontplot=[],rangess=None,Jranges=None,MNL=[5,5]):
	...
	labels = [r'$\mathrm{St\"aclel\quad Fudge}$',r'O2GF']
	...
	plt.subplots_adjust(hspace=0.,wspace=0.6) #compared position of figs
	...
	for N,i in enumerate([0,2]):
		...
		if i in dontplot:
			continue
		l1, = a[0].plot((times-T[0])/TR,results.T[i*2+1],label=labels[i/2],ls=dashed[i],lw=0.5,color=colors[i])
		l2, = a[1].plot((times-T[0])/TR,results.T[i*2+2],ls=dashed[i],lw=0.2,color=colors[i])
	...
	a[0].set_ylabel(r'$J_R/\mathrm{kpc\,km\,s}^{-1}$',fontsize=8)
	a[1].set_ylabel(r'$J_z/\mathrm{kpc\,km\,s}^{-1}$',fontsize=8)
	...
	#fig(2): plot orbit
	a = plt.subplot2grid((2,4),(0,3),colspan=1,rowspan=3) #2403
	a.plot(R,results.T[19],lw=0.3,color='b') #orbit width #0.5
	a.set_ylabel(r'$z/\mathrm{kpc}$')
	a.set_xlabel(r'$R/\mathrm{kpc}$')
	a.set_aspect('equal')
	a.yaxis.tick_right()
	a.yaxis.set_label_position("right")
	a.set_ylim( 2*min(results.T[19]), 2*max(results.T[19]) ) #y range
	a.annotate(name, xy=(0.95,1.02), xycoords='axes fraction', fontsize=14,
	                horizontalalignment='right', verticalalignment='bottom') #'Thin'
	...
	plt.savefig(results_file+'.action_2method.pdf',bbox_inches='tight')
	return spreads
...
def method_comparison2(results_file,name,dontplot=[],rangess=None,Jranges=None,legend=False,MNL=[5,5]):
	...
	a[2].plot(R,results.T[19],lw=0.5,color='k') #plot section #'k'
...
def find_JRJz_of_resonance(results):
	...
	for i in range(len(results)):
		...
		if results[i][7]/results[i][9] < 4./3.:
			N[3]+=1
	#return list( map(lambda i:(results[i[0]][3]-results[i[0]-1][3])/(results[i[0]][7]/results[i[0]][9]-results[i[0]-1][7]/results[i[0]-1][9])*(i[1]-results[i[0]-1][7]/results[i[0]-1][9])+results[i[0]-1][3], zip(N,OmRes)) )

	print (i,' ',N)
	NNNN=map(intm1, N)
	print (NNNN)
	return list( map(lambda i:(results[i[0]][3]-results[i[0]-1][3])/(results[i[0]][7]/results[i[0]][9]-results[i[0]-1][7]/results[i[0]-1][9])*(i[1]-results[i[0]-1][7]/results[i[0]-1][9])+results[i[0]-1][3], zip(NNNN,OmRes)) ) #zip() return a series of vars; lambda i:i is a hidden-name funcion #why invalid indices??
...
def make_table(files,names,dontplot,Jranges):
	...
	acts[0][0]='Actions'
	spreadsnon=[[0 for x in range(9)] for y in range(5)] #notes: inner9*outer6
	spreadscon=[[0 for x in range(9)] for y in range(3)] #notes: 9*3
	for i in range(len(actmethods)):
		if(i>4):
			spreadscon[i-5][0]=actmethods[i]
		else:
			spreadsnon[i][0]=actmethods[i]
			#notes: fianlly there are only the last three methodsin spreadscon(3); the two split actmethods
	MNL=[[3,3],[5,4],[5,5],[6,7]]
	# MNL=[[2,2],[2,2],[2,2],[2,2]]
	...
	for N,(f,n,d,mnl) in enumerate(zip(files,names,dontplot,MNL)):
		...
		r = method_comparison(f,n,dontplot=d,rangess=5.,Jranges=Jranges[N],MNL=mnl) #notes: has called method_comparison(), and paras come here
		acts[0][1+2*N]=r[0][0]
		acts[0][2+2*N]=r[0][1]
...
if __name__=="__main__":
    #time_plot('many_tori_output.dat', with_res=True, with_boxes=True) #ok
    #time_plot('many_tori_output_james.dat',with_res=True,with_boxes=True) #no file
    make_table(['halo','thick','thin','stream'],['Halo','Thick','Thin','Stream'],[[],[],[5],[5]],[None,None,None,[[290.,340.],[540.,580.]]])
    #genfunc_converg(['thin_gc','thick_gc','halo_gc','stream_gc'],['Thin','Thick','Halo','Stream'])
	
	#make_table('thin', 'Thin7', [], None)
	#genfunc_converg('thin_gc', 'Thin_gc')
	#print 7
