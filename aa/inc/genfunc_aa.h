...
struct Actions_Genfunc_data_structure{
    ...
    */
	Actions_Genfunc_data_structure(double tt, unsigned int nt, 
        unsigned int N_mat,double oe, int NTMAX, double maxtimescale, 
        bool axisymmetric=false, bool output_Sn=false)
        : total_T(tt), N_T(nt),N_matrix(N_mat),orbit_eps(oe), 
        NTMAX(NTMAX), maxtimescale(maxtimescale), axisymmetric(axisymmetric), 
        output_Sn(output_Sn)
    {
        ...
        */
        Actions_Genfunc(Potential_JS *Pot, std::string symmetry = "triaxial",lmn_orb *AF=nullptr,bool la_switch=false)
            :symmetry(symmetry),AF(AF),TargetPot(Pot),la_switch(la_switch){
	            // printf("Actions_Genfunc::Actions_Genfunc() called.\n"); //gjy add
