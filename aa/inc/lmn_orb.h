...
/// ellipses(拟合椭圆?) to these orbits to find alpha and beta respectively.
///
//============================================================================
...
/*! Interface for using triaxial St\"ackel fudge apparatus(translation: 仪器)
...
class lmn_orb: public Action_Finder{
	...
	public:
        ...
        /*!
          ...
          \param ym -- minimum intermediate (y) intercept value //ym,yn任意选的??
          ...
          \param s -- output file
        */
		lmn_orb(Potential_JS *pot, double ym=0.05, double yn=80., int NEE=32,
			bool use_log_grid=false, bool use_acts=false, std::string s="")
			: Pot(pot),ymin(ym),ymax(yn),NE(NEE), 
			name(s),use_log_grid(use_log_grid),use_acts(use_acts)
		{
			DEBUG_PRINT_V0d(1, ymin, "ymin");
			E0 = Pot->Phi({0.,ymin,0.});
			Emax = Pot->Phi({0.,ymax,0.}); //gjy note: axisymmetric like, ymax
			DEBUG_PRINT_V1d(1, (VecDoub){ymin, E0, ymax, Emax}, "ymin, E0, ymax, Emax of E_grid"); //gjy add
			// fillDeltagrids(name!=""?name+".Delta_lmn":"Delta_lmn.tmp"); //original code
        ...
        */
		...
		VecDoub actions(const int& ID, const double& t, const VecDoub& x_foci, void* params=nullptr);
        ...
        */
		...
		VecDoub ABC_presentCalculation; //gjy add
};
...
struct root_struct_action{
	...
	// Orbit *orbit;
	SymplecticOrbit *orbit; //gjy changed
	int swit;
	// root_struct_action(Actions_TriaxialStackel_Fudge *ATSF, Orbit *orbit, int swit)
	root_struct_action(Actions_TriaxialStackel_Fudge *ATSF, SymplecticOrbit *orbit, int swit) //gjy changed
...
struct root_struct_actions{
	...
	root_struct_actions(Potential_JS *Pot, SymplecticOrbit *orbit) //gjy changed
		:Pot(Pot),orbit(orbit){}
};
