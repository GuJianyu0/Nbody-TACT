...
//gjy add
/*! Action finding in spherical potential */
class Actions_Spherical_DataPotential : public Action_Finder{
    private:
        VecDoub find_limits(double r, double E, double L);
        Potential_JS *Pot;
        double dr0dH(double r, double L);
    ...
    public:
        //! Actions_Spherical constructor
        /*!
          \param Pot Spherical potential (Potential_JS)
        */
        Actions_Spherical_DataPotential(Potential_JS *Pot): Pot(Pot){};
        //! reset potential
        inline void reset_sph(Potential_JS *pot){Pot=pot;}
        //! Finds actions
        /*!
          \param x phase-space point (x,v)
          \param params -- does nothing
...
/*! Helper structure for finding limits of orbit in spherical potential */
struct Actions_Spherical_DataPotential_limits_struct{
    Potential_JS *Pot;
    double E, L;
    Actions_Spherical_DataPotential_limits_struct(Potential_JS *Pot, double E, double L)
    : Pot(Pot), E(E), L(L){}
};

/*! Helper structure for integrating action integrand in spherical potential */
struct Actions_Spherical_DataPotential_data_struct{
    Potential_JS *Pot;
    double E, L, Lz, Delta, taubar, dDelta, dtaubar;
    Actions_Spherical_DataPotential_data_struct(Potential_JS *Pot, double E, double L, double Lz, double Delta, double taubar, double dDelta=0., double dtaubar=0.)
    : Pot(Pot), E(E), L(L), Lz(Lz), Delta(Delta), taubar(taubar), dDelta(dDelta), dtaubar(dtaubar){}
};
