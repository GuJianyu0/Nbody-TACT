...
int main(int argc, char*argv[]){
	...
	for(unsigned i=0;i<6;++i){
		X[i]=atof(argv[i+1]);
		std::cout<<"output arguments: "<<argc<<' '<<argv[i]<<' '<<X[i]<<std::endl;
	}
	Orbit O(&Pot); //in stackel_fit.cpp
	

	// Fudge
	Actions_AxisymmetricStackel_Fudge AA(&Pot,100.); //in stackel_aa.h
