...
VecDoub Actions_Genfunc::actions(const VecDoub &x, void *params){
    ...
    Actions_Genfunc_data_structure AAR = 
        // Actions_Genfunc_data_structure(timescale*4., 120, 6, //gjy comment: original
        Actions_Genfunc_data_structure(timescale*8., 300, 8, //gjy changed
    ...
    orbit.integrate(x,AA->total_T,AA->stepsize,false); //gjy comment: while(timescale*8-0>0) goes infinite loop for some Nbody potential by direct summation
    ...
    if(
      ...
      (total_loop==0 and std::any_of(b.begin(),b.begin()+3,[](double i){return i<0;}))
        ...
        or fac_of_two
            ...
            fatal=true;
            need_to_repeat=true;
            printf("AGaa: b_012 = %f %f %f, fac_of_two = %d\n", b[0], b[1], b[2], fac_of_two); //gjy add: negative
    }
    ...
    //gjy note: repeating and endless loop for Nbody potential
    if(need_to_repeat)
        if(AA->N_T<AA->NTMAX and AA->total_T<AA->maxtimescale){
            Actions_Genfunc_data_structure AA2 = *AA;
            delete ToyAct;
