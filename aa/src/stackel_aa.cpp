...
VecDoub Actions_AxisymmetricStackel::actions(const VecDoub& x, void *params){
	...
	VecDoub limits = find_limits(tau,integrals); //gjy comment: integrals fitstly and then limits
	VecDoub actions(3,0.);
	// JR
...
void Actions_AxisymmetricStackel_Fudge::integrals(const VecDoub& tau){
	...
	Kt[0] = -2.*(a+tau[0])*(c+tau[0])*pl +(tau[0]+c)*E -(tau[0]+c)/(tau[0]+a)*I2 +chi_lam(tau); //gjy comment: see formula
	Kt[1] = -2.*(a+tau[2])*(c+tau[2])*pn +(tau[2]+c)*E -(tau[2]+c)/(tau[2]+a)*I2 +chi_nu(tau);

	// printf("AASF::integrals(): Kt[0] = %f, Kt[1] = %f\n", Kt[0], Kt[1] ); //gjy add
	// printf("tau: "); print_vec(tau);
	// printf("a = %f, c = %f\n", a, c);
...
VecDoub Actions_AxisymmetricStackel_Fudge::find_limits(const VecDoub& tau){
	...
	if(ptau2ROOT_AxiSym_Fudge(lambda, &RS_l)>=0.0){
		...
		if((laminner+CS->alpha())>tiny_number) //gjy comment: after the last while(){laminner}
			limits.push_back(
			    RF.findroot(&ptau2ROOT_AxiSym_Fudge,laminner,lambda,&RS_l));
		else limits.push_back(-CS->alpha()); //gjy comment: i.e. l<-a+tiny, then pushback -a; choose the bigger

		limits.push_back(RF.findroot(&ptau2ROOT_AxiSym_Fudge,lambda,lamouter,&RS_l));
		if(fabs(limits[0]-limits[1])<TINY){
		    if(ptau2ROOT_AxiSym_Fudge(lambda-5*TINY, &RS_l)>0.0) //gjy comment: now pt2(li)~0 and pt2(l-5T)>0, root in the interval
