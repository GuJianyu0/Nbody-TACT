...
static double *dmatrix_SF(int n){
    ...
    //gjy comment: different from VecDoub: this is a function that creates and returns an array (base address) of length n (all zeros)
...
double Stackel_Fitted_Potential::chi(double lambda, double nu){
    //gjy comment: chi(lam, nu), analytical potential, StackelOblate_PerfectEllipsoid::Phi_tau(const VecDoub& tau) in potential.cpp
...
double Stackel_Fitted_Potential::BigNu(double nu){
    ...
    //gjy comment: de Zeeuw 1988 weight
...
void Stackel_Fitted_Potential::find_limits(VecDoub x){
    ...
    while (alldone1*alldone2*alldone3*alldone4==0){
        ...
        if(steps%5==0 && steps<=STEPMAX){
            ...
            newalpha(((n-1)*alpha()+new_alpha)/(double)n); //gjy comment: average
            n++;
        }
        taunew = xv2tau(ynew);
...
void Stackel_Fitted_Potential::fit_potential(VecDoub x){
    ...
    } //gjy comment: the effect is to let lim0<=lim1
    if(fabs(limits[2]-limits[3])<SMALL) limits[3]+=SMALL; //gjy comment: the effect is to let lim2-lim3>small
    ...
    //gjy comment: in Cuba library, this step is an integration operation
    ...
    lambdagrid=dmatrix_SF(DATAPOINTS); //gjy comment: only creation
    ...
    for(int i=0;i<DATAPOINTS;i++){
        lambdagrid[i] = limits[0]+(double)i*(limits[1]-limits[0])/(double)(DATAPOINTS-1); //gjy comment: lambda uses linear interpolation
    ...
    spline(lambdagrid, flamgrid, DATAPOINTS, 1e30, 1e30, y2Lamgrid); //gjy comment: cubic-spline interpolation; all input parameters are references (addresses)
    spline(nugrid, fnugrid, DATAPOINTS, 1e30, 1e30, y2Nugrid);
}

Actions_StackelFit::Actions_StackelFit(Potential_JS *Pot, double eps){
    //gjy comment: *Pot here is exactly the actually felt potential
...
VecDoub Actions_StackelFit::actions(const VecDoub& x, void*params){
    ...
    SFP->fit_potential(x);
    Actions_AxisymmetricStackel AS(SFP); //gjy comment: AS is the entity of the model axisymmetric potential
    return AS.actions(x);
}
