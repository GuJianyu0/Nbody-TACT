...
// std::cout<<"here12 E_delta[i] = "<<E_delta[i]<<"\n";//gjy add
        double R = .9*Pot->R_E(E_delta[i]); //gjy comment: bounded radius
// std::cout<<"here13 R(E_delta) = "<<R<<"\n";//gjy add
        VecDoub DeltaResult(2,0.);
        for(double j=0;j<NL;j++){
            L_delta[i][j] = Pot->L_circ(R)*(j*0.8/(double)(NL-1)+0.001); //gjy comment: ?? forces
// std::cout<<"here14 NL j = "<<j<<",,,,,,,,,,,,,,,,,,,,,,\nL_delta[i][j]="<<L_delta[i][j]<<", find_best_delta start!!\n";//gjy add //gjy comment: the below is slow and often go back
            find_best_delta DD(Pot, E_delta[i], L_delta[i][j]);
            DeltaResult = DD.delta(R); //gjy note: this find_best_delta DD.delta() sometimes has erorrs for Nbody potential: (At x=0) stepsize underflow in rkqs
