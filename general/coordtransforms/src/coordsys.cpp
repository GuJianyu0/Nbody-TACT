...
VecDoub ConfocalEllipsoidalCoordSys::tau2PSQ(const VecDoub& tau){ //gjy add
	...
	return {tau[0], tau[1], tau[2], P2, Q2, R2};
}
...
// VecDoub ConfocalEllipsoidalCoordSys::xv2tp_GF(const VecDoub& x){
// 	/* Calculates (lambda,...; P^2 l_dot^2,...)(tau) given 6D x vector */ //gjy add
// 	auto tau = xv2tau(x);
// 	double pl =  x[3]/2	*sqrt((tau[1]+Alpha)	*(tau[2]+Alpha)/(Alpha-Beta)*(Alpha-Gamma)	*(tau[0]+Alpha)) 
// 				+x[4]/2	*sqrt((tau[1]+Beta)		*(tau[2]+Beta)/(Beta-Alpha)*(Beta-Gamma)	*(tau[0]+Beta)) 
// 				+x[5]/2	*sqrt((tau[1]+Gamma)	*(tau[2]+Gamma)/(Gamma-Alpha)*(Gamma-Beta)	*(tau[0]+Gamma));
// 	double pm =  x[3]/2	*sqrt((tau[2]+Alpha)	*(tau[0]+Alpha)/(Alpha-Beta)*(Alpha-Gamma)	*(tau[1]+Alpha)) 
// 				+x[4]/2	*sqrt((tau[2]+Beta)		*(tau[0]+Beta)/(Beta-Alpha)*(Beta-Gamma)	*(tau[1]+Beta)) 
// 				+x[5]/2	*sqrt((tau[2]+Gamma)	*(tau[0]+Gamma)/(Gamma-Alpha)*(Gamma-Beta)	*(tau[1]+Gamma));
// 	double pn =  x[3]/2	*sqrt((tau[0]+Alpha)	*(tau[1]+Alpha)/(Alpha-Beta)*(Alpha-Gamma)	*(tau[2]+Alpha)) 
// 				+x[4]/2	*sqrt((tau[0]+Beta)		*(tau[1]+Beta)/(Beta-Alpha)*(Beta-Gamma)	*(tau[2]+Beta)) 
// 				+x[5]/2	*sqrt((tau[0]+Gamma)	*(tau[1]+Gamma)/(Gamma-Alpha)*(Gamma-Beta)	*(tau[2]+Gamma));
// 	return {tau[0], tau[1], tau[2], pl, pm, pn};
// }

// VecDoub ConfocalEllipsoidalCoordSys::xv2tp_other(const VecDoub& x){
// 	/* Calculates (lambda,...; P^2 l_dot^2,...)(tau) given 6D x vector */ //gjy add
// 	auto tau = xv2tau(x);
// 	double P2 = (tau[0]-tau[1])*(tau[0]-tau[2])/(4.*(tau[0]+Alpha)*(tau[0]+Beta)*(tau[0]+Gamma));
// 	double Q2 = (tau[1]-tau[2])*(tau[1]-tau[0])/(4.*(tau[1]+Alpha)*(tau[1]+Beta)*(tau[1]+Gamma));
// 	double R2 = (tau[2]-tau[0])*(tau[2]-tau[1])/(4.*(tau[2]+Alpha)*(tau[2]+Beta)*(tau[2]+Gamma));
// 	return {tau[0], tau[1], tau[2], sqrt(P2)*tau[3], sqrt(Q2)*tau[4], sqrt(R2)*tau[5]};
// }

VecDoub ConfocalEllipsoidalCoordSys::xv2tp(const VecDoub& x){
	/* Calculates (lambda,...; P^2 l_dot^2,...)(tau) given 6D x vector */ //gjy add
	auto tau = xv2tau(x);
	return tau2tp(tau);
}

VecDoub ConfocalEllipsoidalCoordSys::tau2tp(const VecDoub& tau){
	/* Calculates (lambda,...; P^2 l_dot^2,...)(tau) given 6D tau vector */ //gjy add
	double P2 = (tau[0]-tau[1])*(tau[0]-tau[2])/(4.*(tau[0]+Alpha)*(tau[0]+Beta)*(tau[0]+Gamma));
	double Q2 = (tau[1]-tau[2])*(tau[1]-tau[0])/(4.*(tau[1]+Alpha)*(tau[1]+Beta)*(tau[1]+Gamma));
	double R2 = (tau[2]-tau[0])*(tau[2]-tau[1])/(4.*(tau[2]+Alpha)*(tau[2]+Beta)*(tau[2]+Gamma));
	return {tau[0], tau[1], tau[2], P2*tau[3], Q2*tau[4], R2*tau[5]};
}
