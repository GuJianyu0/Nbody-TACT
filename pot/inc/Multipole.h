...
public:
    ...
    double density_sph(const VecDoub& r){ //gjy note: m2 r2
...
class Density_DoublePowerLaw: public TriaxialDensity{ //gjy add
private:
    double rhos, rs;
    double GM;
    VecDoub a;
...
public:
    Density_DoublePowerLaw(double rhos, double rs, VecDoub a, double powerA, double powerB)
        :TriaxialDensity(a),rhos(rhos),rs(rs),a(a),powerA(powerA),powerB(powerB){
        GM = conv::FPG*rhos*pow(rs,3);
    };
    double density_m2(double m){
        double sqm = sqrt(m);
        // return GM/sqm/(sqm+rs)/(sqm+rs)/conv::FPG; //NFW
        double rqn = sqm/rs; //but cannot zero
        return rhos*pow(rqn, -powerA)*pow(1.+rqn, -powerB);
    }
    double Phi(const VecDoub& x){
        std::cout<<"This analytical potential is very complicated. Not shown here.\n";
        double r = sqrt(x[0]*x[0]/a[0]/a[0]+
                        x[1]*x[1]/a[1]/a[1]+
                        x[2]*x[2]/a[2]/a[2]);
        return 0.;
...
class Density_Einasto: public TriaxialDensity{ //gjy add
private:
    double rhos, rs;
    VecDoub a;
    double n;
public:
    Density_Einasto(double rhos, double rs, VecDoub a, double powerA)
        :TriaxialDensity(a),rhos(rhos),rs(rs),a(a),n(powerA){
            if(n<0.1 || n>10.){
                std::cout<<
                    "Density_Einasto warning: When using the approximation of normalization coefficient, "
                    "the power index (1./n) of Einasto profile should be in range of [0.1, 10.]. "
                    "But now it it is out the of range.\n";
            }
        };
    double density_m2(double m){
        double sqm = sqrt(m);
        double rqn = sqm/rs;
        double bn = 3.*n-1./3+0.0079/n;
        return rhos*exp( -bn*( pow(rqn, 1./n)-1. ) );
    }
    double Phi(const VecDoub& x){
        std::cout<<"This analytical potential is very complicated. Not shown here.\n";
        double r = sqrt(x[0]*x[0]/a[0]/a[0]+
...
private:
    ...
    std::vector<std::vector<VecDoub>> Phi_grid, rho_grid, dPhi_grid; //gjy note: f[xi][yi][zi]
...
public:
    ...
    */
    MultipoleExpansion(Density *rho, int NR = 50, int NA = 8, int LMAX = -1, int MMAX = -1, 
        double a0=1.,double rmin=0.01,double rmax=100., 
        bool axisymmetric = false, bool triaxial = false, bool flip = true, 
        double err=0., bool vocal=false
    );
    ...
    double Phi(const VecDoub& x, int coor=0) override;
    // virtual double Phi(const VecDoub& x) override;
    VecDoub Forces(const VecDoub& x, int coor=0) override;
};
