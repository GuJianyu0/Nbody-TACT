...
class Orbit_base{
	protected:
		Potential_JS *Pot;
		std::vector<VecDoub> results_;  /*!< Stores orbit integration results*/ //6*N
	...
	public:
		...
		// virtual VecDoub integrate(const VecDoub& x, const double& t_tot, double t_interval, double step_size, bool adaptive=false) = 0; //gjy add
		// virtual double integrate1(double x) = 0;
...
class Orbit: public Orbit_base{
	...
	public:
	    ...
	    */
		...
		// VecDoub integrate(const VecDoub& x, const double& t_tot, double t_interval, double step_size, bool adaptive=false); //gjy add
		// double integrate1(double x); //gjy add
...
class SymplecticOrbit: public Orbit_base{
	...
	public:
	    ...
	    */
		...
		SymplecticOrbit(Potential_JS *P, double fac = 10.):Orbit_base(P),fac(fac){}; //gjy changed: fac
		//! Integrate phase-space point.
	    /*!
	        \param x -- starting phase-space point x=(x,y,z,vx,vy,vz)
	        \param t_interval -- total integration time
