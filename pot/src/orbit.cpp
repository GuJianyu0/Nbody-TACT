...
// 		//gjy comment: orbit O.integrate has results; ode O ~ Potential_JS when initialization.
// 		//\ Calculation is in ode O.step(), which is defined in GSLInterface.h); 
//		//\ The first argument is derivs() with potential of points; the y is the next step of orbit.
// 		for(int i=0;i<6;i++) xf[i]=y[i];
// 		results_.push_back(xf);
// 		start+=step;
// 	}
